...
do
{
    ...
    Console.WriteLine("[5] Optimize motorcycles");
    Console.WriteLine("[6] Show Parking\n");
...
void optomizeMC()
{
    ...
    for (int i = 0; i < parking.Length; i++)
    {
        ...
        {
            string subString = parking[i].ToString();
            int searchMC = subString.IndexOf("MC");
            int searchSeperator = subString.IndexOf("|");
            ...
            if (searchMC != -1 && searchSeperator == -1)
            {
                for (int j = 0; j < parking.Length; j++)
                {
                    if (parking[j] != null)
                    {
                        string secondSubString = parking[j].ToString();
                        int searchOtherMC = secondSubString.IndexOf("MC");
                        int searchOtherSeperator = secondSubString.IndexOf("|");

                        if (searchOtherMC != -1)
                        {
                            if (subString == secondSubString)
                            {
                                continue;
                            }
                            else if (searchOtherSeperator == -1)
                            {
                                string joinedString = subString + "|" + secondSubString;
                                parking[j] = joinedString;
                                parking[i] = null;

                                counter++;

                                Console.WriteLine($"{subString} is now parked together with {secondSubString} at P-Spot {spotNr[j]}");
                                break;
                            }
                            else if (searchOtherSeperator != -1)
                            {
            ...
            if (counter == 0)
            {
                Console.WriteLine("No motorcycles were moved");
            }
        }
